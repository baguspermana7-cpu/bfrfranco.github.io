            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
                <div className="min-w-0 space-y-4">
                    ...
                    <div id="sec-overview" className="grid scroll-mt-24 grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
                        ...
                        {([
                            { label: 'Total Score', value: kpi.engine.score, direction: 'higher', traceId: 'site.totalScore', explainKey: 'site-overall-score', grade: kpi.engine.grade, sub: `${kpi.engine.grade} · ${kpi.engine.label}`, tip: 'Weighted composite of all 10 site factors (models.site.score) on a 0–100 scale — higher is better. The grade chip applies the shared score bands (good ≥70 · watch ≥40 · bad below).' },
                            { label: 'Availability Score', value: kpi.availabilityScore, direction: 'higher', traceId: 'site.availabilityScore', explainKey: 'availability-score', sub: 'power + grid', tip: 'Power-availability sub-score (0–100, higher is better) combining grid reliability and power-infrastructure factors for this site. Weak grids drag the score down and usually demand more on-site generation or storage.' },
                            { label: 'Connectivity Score', value: kpi.connectivityScore, direction: 'higher', traceId: 'site.connectivityScore', explainKey: 'connectivity-score', sub: 'latency + cables', tip: 'Network sub-score (0–100, higher is better) from latency to major hubs and subsea/terrestrial cable diversity. Poor connectivity limits the workloads the site can serve.' },
                            { label: 'Water & Cooling', value: kpi.waterCoolingScore, direction: 'higher', traceId: 'site.waterCoolingScore', explainKey: 'water-cooling-score', sub: 'water + climate', tip: 'Cooling-feasibility sub-score (0–100, higher is better) from water availability/stress and the local climate’s suitability for free cooling. Low scores push designs toward dry coolers or immersion at higher cost.' },
                            { label: 'Risk Score', value: kpi.riskScore, direction: 'lower', traceId: 'site.riskScore', explainKey: 'site-risk-score', sub: 'lower is better', tip: 'Composite natural-hazard risk (0–100) — LOWER is better, and the color scale is inverted accordingly. Driven by seismic, flood, typhoon and related country hazard attributes.' },
                        ] as { label: string; value: number; direction: 'higher' | 'lower'; traceId: string; explainKey: string; grade?: string; sub: string; tip: string }[]).map((k) => (
                            <div key={k.label} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-3">
                                ...
                                <div>
                                    <ScoreValue value={k.value} unit="/100" direction={k.direction} max={100} traceId={k.traceId} explainKey={k.explainKey} tip={k.tip} grade={k.grade} className="text-lg" />
                                </div>
