...
function ScoreRing({ score, band }: { score: number; band: { label: string; sub: string } }) {
    ...
    const color = score >= 85 ? '#00FF88' : score >= 60 ? '#00DDFF' : '#FFAA00';
    return (
    ...
    return (
        <aside className="space-y-4 lg:sticky lg:top-4 self-start">
            <div className="rounded border border-rz-mint/30 bg-rz-mint/5 p-4">
                <h3 className="mb-2 flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-rz-mint">
                ...
                <ul className="space-y-1.5">
                    {derived.insights.map((s, i) => (
                        <li key={i} className="flex gap-1.5 text-[11px] text-slate-700 dark:text-slate-300">
                            <span className="text-rz-mint">✓</span>{s}
                        </li>
                    ))}
                </ul>
            ...
            <div className="rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4">
                ...
                <div className="space-y-1.5 text-xs">
                    <div className="flex items-center justify-between rounded-lg border border-rz-mint/40 bg-rz-mint/10 px-2 py-1.5">
                        <div>
                        ...
                        <span className="rounded-full bg-rz-mint px-1.5 py-0.5 text-[9px] font-semibold text-rz-base">Active</span>
                    </div>
                ...
                <button onClick={() => openPanel?.()} className="mt-2 w-full rounded-lg border border-slate-300 dark:border-slate-700 py-1.5 text-[11px] font-medium text-slate-600 dark:text-slate-300 hover:border-rz-mint">
                    View All Scenarios
                </button>
            </div>
                    ...
                    {derived.sectionQuality.map((s) => (
                        <div key={s.key} className="flex items-center gap-2 text-[11px]">
                            ...
                            <div className="h-1.5 flex-1 rounded bg-slate-100 dark:bg-slate-800">
                                <div className={`h-1.5 rounded ${s.pct === 100 ? 'bg-rz-data' : s.pct >= 60 ? 'bg-rz-info' : 'bg-rz-signal'}`} style={{ width: `${s.pct}%` }} />
                            </div>
            ...
            <button onClick={() => setActiveTab('site')}
                className="flex w-full items-center justify-center gap-1 rounded-lg bg-rz-mint py-2.5 text-sm font-semibold text-rz-base hover:bg-rz-mint/80 transition-colors">
