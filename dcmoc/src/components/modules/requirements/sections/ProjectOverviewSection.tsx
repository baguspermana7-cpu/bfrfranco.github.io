    ...
    return (
        <div className="mt-3 rounded-lg border border-slate-200 dark:border-slate-700/70 bg-slate-50/60 dark:bg-slate-800/40 p-3">
            <div className="flex items-center gap-2 mb-2">
                <ImageIcon className="w-3.5 h-3.5 text-rz-mint" />
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-200">Data Center Image (Dashboard hero)</span>
                <span className={`ml-auto rounded-full px-2 py-0.5 text-[9px] font-medium uppercase tracking-wide ${heroImage ? 'bg-rz-mint/15 text-rz-mint' : 'bg-slate-500/15 text-slate-500 dark:text-slate-400'}`}>
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col gap-1.5 min-w-0">
                    <div className="flex items-center gap-2">
                        <label className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-medium bg-rz-mint hover:bg-rz-mint/80 text-rz-base cursor-pointer transition-colors">
        ...
        <SectionCard num="1.1" title="Project Overview" caption="Basic information about the project and customer" id="sec-overview">
                ...
                <Field label="City / Region" hint="Pilih kota DC-hub (dropdown) atau ketik custom">
                    <>
                        <input list="dc-city-options" value={o.cityRegion} onChange={(e) => set({ cityRegion: e.target.value })}
                            ...
                            className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-sm text-slate-900 focus:border-rz-mint focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-white" />
            ...
            <div className="mt-3">
                ...
                <Field label="IT Workload / Use Case" explainKey="workload" hint="Selected in 1.2 Workload Profile — one source">
                    <a href="#sec-workload" className="inline-flex items-center gap-1.5 rounded-full border border-rz-mint/40 bg-rz-mint/10 px-2.5 py-1 text-[11px] font-medium text-rz-mint hover:bg-rz-mint/20">
                        {USE_CASE_LABELS[o.useCase]}
                        <span className="text-[9px] uppercase text-rz-mint">edit in 1.2 ↓</span>
                    </a>
                </Field>
            </div>
