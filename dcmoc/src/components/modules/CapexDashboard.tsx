...
const CapexDashboard = () => {
    ...
    const onHeroFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setHeroBusy(true);
        try {
            const url = await compressToWebp(f, { maxW: 1280, quality: 0.82 });
            setHeroImage(url);
        } catch (err) {
            console.error('hero compress failed', err);
        }
        setHeroBusy(false);
    };
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full gap-6 p-4 lg:p-6 bg-slate-50 dark:bg-slate-900 lg:overflow-hidden">
            ...
            <div className="w-full lg:w-[420px] flex-shrink-0 flex flex-col gap-4 overflow-y-auto pr-0 lg:pr-2 pb-6 lg:pb-20">
                {/* ─── PROJECT HERO IMAGE (shown on the Executive Dashboard) ─── */}
                <Card>
                    <CardHeader className="pb-3">
                        <CardTitle className="text-sm flex items-center gap-2">
                            <ImageIcon className="w-4 h-4 text-indigo-600" />
                            Project Hero Image
                            <Tooltip content="Optional render/photo shown on the Executive Dashboard Project Summary. Compressed to WebP in your browser so it loads fast; falls back to the default DC-OS render." />
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2.5">
                        <div className="relative h-32 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                            <img src={heroImage || '/dcmoc/hero-default.webp'} alt="Project hero preview" className="absolute inset-0 w-full h-full object-cover" />
                            {heroBusy && <div className="absolute inset-0 flex items-center justify-center bg-black/40 text-white text-xs">Compressing…</div>}
                        </div>
                        <div className="flex items-center gap-2">
                            <label className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium bg-indigo-600 hover:bg-indigo-500 text-white cursor-pointer transition-colors">
                                <Upload className="w-3.5 h-3.5" /> Upload Image
                                <input type="file" accept="image/*" className="hidden" onChange={onHeroFile} />
                            </label>
                            {heroImage && (
                                <button onClick={() => setHeroImage(null)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                    <RotateCcw className="w-3.5 h-3.5" /> Default
                                </button>
                            )}
                        </div>
                        <p className="text-[10px] text-slate-400 dark:text-slate-500">
                            {heroImage ? `Custom · WebP · ~${Math.round(dataUrlBytes(heroImage) / 1024)} KB` : 'Using default DC-OS render'}
                        </p>
                    </CardContent>
                </Card>
