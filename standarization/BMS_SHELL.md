...
| 5 | **v1.42.4** | `chiller-plant.html` CHW P&ID (first cross-page adoption) | rz-line-model + rz-breaker-symbols script tags added. drawPid() ports **+18 lines** (4 loops × 4 sup/ret legs + 2 distribution headers). Loop state binds to engine `evalLoop().level` (`fault` when ALARM). **EPMS_Telemetry.html intentionally untouched** per owner mandate ("jangan merusak EPMS DC Conventional ya, enhance bole"). |
| 6 | **v1.42.5** | `water-system.html` + `fire-system.html` (static SVG P&IDs) | rz-line-model script tag added. **10 water + 14 fire = 24 pipes** tagged inline with full schema (no JS port — direct `data-*` on `<path class="pipe-base">`). Probe `waitUntil` relaxed to `'load'` to avoid networkidle2 timeouts on flow-animation pages. |
| 7 | **v1.43.0** | `datahallAI.html` right-side inspector (review doc-27 §3.2 P0) | NEW `js/rz-inspector.js` — 6-tab panel (Live/Capacity/Deps/Alarms/Trend/Maint). Auto-attaches to `[data-rz-line]` + `[data-rz-breaker]`. NEW `standarization/INSPECTOR.md`. Probe extended (21 pass). chiller-plant + water-system + fire-system inspector load deferred to v1.43.1. EPMS deferred per owner mandate. |
| 8 | **v1.43.1** | inspector cross-page parity — `chiller-plant.html` + `water-system.html` + `fire-system.html` | One-line `<script src>` per page (additive). Probe extended to 27 pass (4 inspector pages × `RZInspector exposed` + click opens). Visual identical. EPMS still untouched. |
| 9 | **v1.43.2** | Data-quality service — 4 cockpit pages | NEW `js/rz-telemetry-quality.js` — 7 states + page banner + point chip. NEW `standarization/TELEMETRY_QUALITY.md`. Body `data-rz-data-mode="simulated"` + script tag on all 4 inspector pages. Inspector Live tab now renders Data Quality chip. Probe extended to 39 pass (added 3 telemetry-quality assertions per inspector page). EPMS still untouched. |
| 10 | **v1.43.3** | `datahallAI.html` #p-dash KPI tooltips (review doc-27 §3.4 P0) | `updateDashKPI()` appends additive IIFE setting `title` (source+formula+timestamp) on 7 headline KPIs (PUE/WUE/CUE/IT/GPU/NVL72/CDU). **Displayed values byte-identical** — only hover metadata new, accuracy probe confirms no random-basis regression. Probe extended to 40 pass. Owner-exclusion on #p-dash was lifted 2026-05-23. |
| 11 | **v1.43.4** | Alarm state machine + color discipline — 4 cockpit pages (review doc-27 §3.3 P0 + §4.3 P1) | NEW `js/rz-alarm-state.js` — ISA-18.2 7-state machine + `resolveColor()` (status wins over domain) + severity tiers. NEW `standarization/ALARM_STATE.md`. Inspector Alarms tab renders ISA-18.2 chip + summary. Loaded on all 4 inspector pages. Probe 52 pass. **Closes the LAST open P0 in review doc-27.** EPMS untouched. |
