...
## Bento polish pattern (index — editorial/content surfaces)

The homepage is **not** re-skinned to the instrument register — the owner chose to **polish the
existing colourful bento** (emerald + pastel per-card accents + rounded cards + photo + Inter), not
replace its character. This is the canonical pattern for elevating an existing card surface to
premium quality **without** changing its identity. Shipped: dark `v1.43.14`, light `v1.43.18`.
...
> have — the reveal *is* the animation; the index career cards are content cards, not KPI cards.
