...
| **v1.42.4** | `chiller-plant.html` CHW P&ID (first cross-page adoption) | **+30** (v2.5.0: 4 loops × 4 lines + 2 primary headers + 2 risers + 2 distribution headers + 4 halls × 2 branches). Loop state binds to engine `evalLoop()` (`fault` when alarming, `energized` otherwise). EPMS_Telemetry.html intentionally untouched per owner mandate. | `ADOPTION_TARGETS['chiller-plant.html'] = 18` |
| **v1.42.5** | `water-system.html` + `fire-system.html` (static SVG P&ID pipes) | **+24** (10 water + 14 fire). Pipes tagged inline (mains/raw/filter/pump/UV/dom-tank/makeup/blowdown for water; landlord/suction/discharge/jockey/diesel/wet/preaction/N2-header/5 N2 branches for fire). v2.6.0: the fire sheet was recomposed (detection layer, valves/check valves/instruments, test header, hydrant riser, arrowheads via `marker-end`) with the same 13 tagged lines — instrument symbols are untagged glyphs, not lines. | `ADOPTION_TARGETS['water-system.html'] = 10`, `['fire-system.html'] = 14` |
