...
| v1.46.5 | 06-26 | `css/rz-article-dark.css` (all articles + hubs) | **Editorial reading-experience overhaul (uiux-review).** Capped prose measure to 68ch (was ~142ch); forced day body to IBM Plex (day block lacked the rule → Inter); explicit heading scale (h2 600 / h3 500, ratio ~1.55) + rhythm; removed the day hero amber wash (washed out saturated heroes); figcaption `!important` mono; section-number kickers (mono 01/02/03 on h2); lead paragraph; figure framing. Verified 37 pages 0 light-on-dark/0 err/0 over-wide. |
| v1.46.4 | 06-26 | `css/rz-article-dark.css` | **Editorial refinements + day-accent contrast fix.** Day `--rz-art-accent` was overridden by each article's inline `#E8B563` (fails on white) → forced readable `#b45309` with `!important`. Refined body links (amber underline), replaced the pull-quote 2px side-stripe (impeccable ban) with a hanging Fraunces quote glyph, mono figcaptions, amber selection. |
| v1.49.3 | 06-27 | changelog.html + gate | **Both-mode enforcement.** Fixed the inverse bug: `changelog.html` had `[data-theme="light"]` card rules but no `[data-theme="light"] body` → stuck dark in light mode (light navbar over dark body). Added the light body to the generator template (`build-changelog-html.py`). `achievements.html` = intentionally dark-only (no light palette) → correct. **`tools/audit-dark-coverage.mjs` now checks BOTH themes** — fails STUCK-DARK-IN-LIGHT (a page with a light palette must render light in light mode); dark-only pages skip the light check. Gate CLEAN across 114 pages in both modes. |
| v1.48.2 | 06-27 | both-mode text audit | **Comprehensive light+dark contrast audit of all 114 pages** (3 probes — 2 DOM + 1 pixel-accurate via sharp, sampling real rendered backgrounds — + ~14 pages screenshot-verified in both modes). Genuine finding: `.cmp-badge-a/-b` accent-on-same-tint (~2.5 contrast) → solid accent + white (5 compare pages). **Everything else is readable**: probe flags were false positives — light text on colored heroes (intentional), auth-gate dimming (gated labs/cockpits show dimmed content behind the Sign-In modal), gradient/photo card backgrounds, dim-by-design mono labels/captions/chart-annotations, and **dark-only instrument cockpits** (`datahall/chiller/water/fire/fuel/ict` have NO light toggle/palette — dark by the instrument standard, so auditing them in "light" is moot). Body text/headings/lists/tables render readable site-wide in both modes. |
| v1.48.1 | 06-26 | 13 content pages + enforcement gate | **White-body-in-dark fixed + standard made self-enforcing.** Root cause = the `:root, [data-theme="light"]` cascade bug (the `:root,` selector matches in all themes and overrode the dark vars) on 11 pages (cdu-calculator/hub/selection-guide, 5× compare-*, fire-calculator/checklist, pln-sumatra) → changed to `:root:not([data-theme="dark"])` (flips bg **and** body text dark). + tia-942 (added dark var block + .calc-disclaimer) + tier-advisor (.calc-disclaimer). **New gate `tools/audit-dark-coverage.mjs --strict`** (render: fails white body/content in dark + flags the `:root,` bug) added to the ship-audit suite + documented in CLAUDE.md + DARK_MODE_STANDARD.md. Gate CLEAN across 114 pages. |
