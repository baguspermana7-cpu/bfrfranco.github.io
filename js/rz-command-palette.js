  ...
  function openSearch() {
    ...
    if (!searchData) {
      ...
      var loadData = function () {
        /* v1.60.0: merge the glossary term corpus (search-terms.json, generated by
           tools/build-explain-db.py) so all 354 terms are palette-searchable. Best-effort. */
        fetch('search-index.json').then(function (r) { return r.json(); }).then(function (pages) {
          fetch('search-terms.json').then(function (r) { return r.json(); }).then(function (terms) {
            initFuse(pages.concat(terms || []));
          }).catch(function () { initFuse(pages); });
        }).catch(function () {});
