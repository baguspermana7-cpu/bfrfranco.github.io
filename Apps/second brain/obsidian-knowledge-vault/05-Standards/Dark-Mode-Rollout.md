...
- **v1.49.3** — **Both-mode enforcement (the inverse bug).** The both-mode structural check (body bg must match the active theme) caught `changelog.html` **stuck dark in light mode** — it had `[data-theme="light"]` card rules but no `[data-theme="light"] body`, so light mode rendered a light navbar over a dark body. Fixed in the generator template (`build-changelog-html.py`) — added the light body/surface; hero band intentionally stays dark. `achievements.html` is intentionally **dark-only** (no light palette) → correct as-is. **`tools/audit-dark-coverage.mjs` now enforces BOTH themes:** fails STUCK-DARK-IN-LIGHT (any page declaring a light palette must render a light body in light mode); pages with no light palette are dark-only and skip the light check. Gate CLEAN across 114 pages in both modes. So the standard is now self-enforcing in **both** directions (no white-body-in-dark AND no stuck-dark-in-light).
