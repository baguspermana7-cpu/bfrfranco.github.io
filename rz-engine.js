...
(function (root) {
    ...
    var DATA = {
        ...
        dossier: {
            /* EPC permitting register — STANDARD-PRACTICE reference (jurisdiction-agnostic;
             * the AHJ + statutory durations vary by country — validate locally). `risk`
             * kept as the approval-risk alias for the chip. Grouped by lifecycle phase. */
            permittingMatrix: [
                { permit: 'Zoning / Land-Use & Site Plan Approval', phase: 'pre-construction', authority: 'Planning / zoning authority', purpose: 'Confirm the site is zoned for a data-center/industrial use + site-plan compliance.', requiredDocs: ['Site plan', 'Zoning application', 'Land title / lease', 'Traffic & utility impact'], submissionFlow: 'pre-app → submit → public/agency review → conditions → approve', durationWk: 10, dependency: 'Secured site + concept masterplan', risk: 'med', inspection: 'Site-plan compliance check', renewal: 'one-off', standard: 'local planning code' },
                { permit: 'Environmental Permit / EIA', phase: 'pre-construction', authority: 'Environmental agency', purpose: 'Assess + license environmental impact (emissions, water, noise, generators).', requiredDocs: ['EIA / EIS study', 'Emissions inventory', 'Stormwater plan', 'Noise assessment'], submissionFlow: 'scoping → EIA study → submit → public consultation → RFI → approve', durationWk: 20, dependency: 'Baseline environmental study', risk: 'high', inspection: 'Environmental audit + monitoring points', renewal: 'periodic (annual reporting)', standard: 'local EIA regulation' },
                { permit: 'Building / Construction Permit', phase: 'pre-construction', authority: 'Local building authority / AHJ', purpose: 'Authorize construction against code-compliant design.', requiredDocs: ['Stamped design drawings', 'Structural calcs', 'MEP drawings', 'Code-compliance report'], submissionFlow: 'submit design → plan check → RFI → approve → permit issue', durationWk: 12, dependency: 'Approved design + zoning', risk: 'high', inspection: 'Staged construction inspections', renewal: 'per-phase (extensions)', standard: 'local building code' },
                { permit: 'Fire Department / Life-Safety Approval', phase: 'pre-construction', authority: 'Fire authority', purpose: 'Approve fire strategy, detection, suppression + egress.', requiredDocs: ['Fire strategy report', 'Detection & suppression drawings', 'Cause-&-effect matrix', 'Egress plan'], submissionFlow: 'submit strategy → review → RFI → approve → discharge-test witness', durationWk: 8, dependency: 'Fire strategy + C&E matrix', risk: 'med', inspection: 'Suppression discharge + alarm witness test', renewal: 'annual re-certification', standard: 'NFPA 75/76/2001 / local' },
                { permit: 'Grid Interconnection / Utility Connection Agreement', phase: 'pre-construction', authority: 'Electric utility / TSO', purpose: 'Secure firm grid capacity + connection terms (the schedule-critical long pole).', requiredDocs: ['Load study', 'Connection application', 'SLD', 'Capacity reservation payment'], submissionFlow: 'apply → system-impact study → offer → agreement → build', durationWk: 52, dependency: 'Confirmed IT-MW + site power demand', risk: 'high', inspection: 'Metering + protection commissioning witness', renewal: 'one-off (capacity charges ongoing)', standard: 'utility connection code' },
                { permit: 'Excavation / Earthworks Permit', phase: 'construction', authority: 'Building authority / utilities', purpose: 'Authorize excavation with utility-strike + shoring safety.', requiredDocs: ['Excavation plan', 'Utility survey (dial-before-dig)', 'Shoring design', 'Dewatering plan'], submissionFlow: 'utility locate → submit → approve → permit-to-dig', durationWk: 3, dependency: 'Site survey + utility mapping', risk: 'med', inspection: 'Shoring + trench safety inspection', renewal: 'per-activity', standard: 'local safety code' },
                { permit: 'Crane / Lifting Permit', phase: 'construction', authority: 'Site safety / labour authority', purpose: 'Authorize heavy lifts (transformers, gensets, CDU) with lift plan.', requiredDocs: ['Lift plan', 'Crane cert', 'Rigger competency', 'Ground-bearing check'], submissionFlow: 'lift plan → review → permit-to-lift', durationWk: 1, dependency: 'Rigging study + crane mobilization', risk: 'med', inspection: 'Pre-lift toolbox + rigging check', renewal: 'per-lift', standard: 'LOLER / local lifting reg' },
                { permit: 'Confined-Space Entry Permit', phase: 'construction', authority: 'Site HSE', purpose: 'Control entry to tanks/pits/plenums (atmosphere + rescue).', requiredDocs: ['Gas-test record', 'Rescue plan', 'Entry log'], submissionFlow: 'gas test → permit → entry → close-out', durationWk: 0, dependency: 'HSE plan', risk: 'high', inspection: 'Atmospheric monitoring', renewal: 'per-entry (per-shift)', standard: 'OSHA 1910.146 / local' },
                { permit: 'Hot-Work Permit', phase: 'construction', authority: 'Site HSE / fire warden', purpose: 'Control welding/cutting/grinding fire risk.', requiredDocs: ['Hot-work checklist', 'Fire-watch assignment'], submissionFlow: 'assess → permit → fire-watch → close-out', durationWk: 0, dependency: 'HSE plan', risk: 'med', inspection: 'Fire-watch + 60-min post-work watch', renewal: 'per-shift', standard: 'NFPA 51B / local' },
                { permit: 'Working-at-Height Permit', phase: 'construction', authority: 'Site HSE', purpose: 'Control fall risk (roof plant, scaffold, MEWP).', requiredDocs: ['Fall-protection plan', 'Scaffold/MEWP cert', 'Competency records'], submissionFlow: 'assess → permit → work → inspect', durationWk: 0, dependency: 'HSE plan', risk: 'med', inspection: 'Scaffold/anchor inspection', renewal: 'per-activity', standard: 'local WAH reg' },
                { permit: 'Traffic-Management Plan', phase: 'construction', authority: 'Roads / municipal authority', purpose: 'Manage abnormal-load delivery + site traffic on public roads.', requiredDocs: ['TM plan', 'Abnormal-load route survey', 'Escort arrangements'], submissionFlow: 'submit → police/roads approve → escort', durationWk: 4, dependency: 'Delivery schedule (long-lead route)', risk: 'low', inspection: 'Route/escort compliance', renewal: 'per-delivery window', standard: 'local roads authority' },
                { permit: 'Temporary Power Connection', phase: 'construction', authority: 'Electric utility', purpose: 'Site construction power ahead of permanent supply.', requiredDocs: ['Temp-supply application', 'Temp SLD', 'Load estimate'], submissionFlow: 'apply → meter → energize', durationWk: 6, dependency: 'Utility application', risk: 'med', inspection: 'Temp-board earth + RCD test', renewal: 'until permanent supply', standard: 'utility temp-supply code' },
                { permit: 'Temporary Water Connection', phase: 'construction', authority: 'Water utility', purpose: 'Construction water (concrete, dust, welfare).', requiredDocs: ['Temp-water application', 'Backflow-prevention detail'], submissionFlow: 'apply → connect', durationWk: 3, dependency: 'Utility application', risk: 'low', inspection: 'Backflow-prevention check', renewal: 'until permanent supply', standard: 'local water utility' },
                { permit: 'Fuel Storage Permit', phase: 'construction', authority: 'Fire authority / environmental', purpose: 'License bulk diesel/HVO storage for gensets.', requiredDocs: ['Tank design', 'Secondary-containment detail', 'Spill-response plan'], submissionFlow: 'submit → review → approve → tank-test witness', durationWk: 10, dependency: 'Tank design + bund sizing', risk: 'med', inspection: 'Tank integrity + bund + spill drill', renewal: 'annual', standard: 'NFPA 30 / local' },
                { permit: 'Hazardous-Material Handling', phase: 'construction', authority: 'Environmental / fire authority', purpose: 'Control refrigerants, coolants, batteries, clean-agent.', requiredDocs: ['SDS register', 'Storage/handling plan', 'Battery/refrigerant inventory'], submissionFlow: 'inventory → plan → approve', durationWk: 4, dependency: 'Equipment coolant/refrigerant selection', risk: 'med', inspection: 'Hazmat storage + labelling audit', renewal: 'annual', standard: 'local hazmat reg' },
                { permit: 'Water Supply & Wastewater / Discharge', phase: 'energization', authority: 'Water utility / municipality', purpose: 'Permanent make-up water + condensate/blowdown discharge.', requiredDocs: ['Water balance', 'Discharge-quality plan', 'Trade-effluent application'], submissionFlow: 'apply → review → consent', durationWk: 12, dependency: 'Cooling water balance + discharge plan', risk: 'med', inspection: 'Discharge-quality sampling', renewal: 'periodic consent', standard: 'local utility / trade-effluent' },
                { permit: 'MV/HV Energization Approval', phase: 'energization', authority: 'Utility / electrical inspectorate', purpose: 'Authorize live energization of MV/HV switchgear + transformers.', requiredDocs: ['As-built SLD', 'Protection settings', 'Test reports', 'Compliance cert'], submissionFlow: 'submit tests → inspect → approve → energize', durationWk: 16, dependency: 'SLD + protection coordination + SAT', risk: 'high', inspection: 'Protection + earthing + insulation witness', renewal: 'one-off', standard: 'IEC/IEEE + local' },
                { permit: 'Telecommunications / Carrier Entry', phase: 'energization', authority: 'Telecom regulator / carriers', purpose: 'License diverse carrier fiber entry to the MMR.', requiredDocs: ['Route survey', 'Carrier agreements', 'Wayleave / duct permit'], submissionFlow: 'survey → agreements → build → accept', durationWk: 12, dependency: 'Route survey + carrier agreements', risk: 'low', inspection: 'Fiber entry + MMR acceptance', renewal: 'one-off', standard: 'local telecom' },
                { permit: 'Occupancy / Completion Certificate', phase: 'occupation-operation', authority: 'Building authority', purpose: 'Certify the building is safe + code-compliant to occupy.', requiredDocs: ['Final inspection sign-off', 'As-built drawings', 'Fire cert', 'Test certs'], submissionFlow: 'final inspection → snag → certificate', durationWk: 6, dependency: 'Completed construction + as-builts', risk: 'med', inspection: 'Final multi-discipline inspection', renewal: 'one-off', standard: 'local building code' },
                { permit: 'Operational License / Registration', phase: 'occupation-operation', authority: 'Business / data authority', purpose: 'License to operate the facility as a data center.', requiredDocs: ['Occupancy cert', 'Compliance certs', 'Business registration'], submissionFlow: 'submit → review → license', durationWk: 8, dependency: 'Occupancy + compliance certs', risk: 'med', inspection: 'Compliance audit', renewal: 'annual', standard: 'local jurisdiction' },
                { permit: 'Periodic Authority Inspection (electrical/fire/lift)', phase: 'occupation-operation', authority: 'AHJ / inspectorate', purpose: 'Statutory in-service re-inspection of critical systems.', requiredDocs: ['Prior cert', 'Maintenance records', 'Test reports'], submissionFlow: 'schedule → inspect → certify', durationWk: 2, dependency: 'PM records + test history', risk: 'low', inspection: 'Electrical/fire/lift statutory inspection', renewal: 'annual / statutory interval', standard: 'local statutory' }
            ],
            designBasis: [
 ...
 * quote, gate-enforced). Do NOT hand-edit — rerun the corpus pipeline. */
         ...
         * curated 445-part spare-parts catalog (js/spares-parts-catalog.js, itself
         * generated from the gitignored sqlite by tools/build-spares-db.py). Values
         * are p50 (median) over the parts in each commodity's catalog subsystem(s):
         * mtbf(yr), mttr(hr), leadWk(ltTyp), costUsd(cTyp), alts, crit(1-10),
         * eol(months), lifecycle mix + part count n. Generated by
         * tools/build-spares-catalog-aggregate.mjs — DO NOT hand-edit; regenerate
         * when the catalog changes. Read via models.spares.catalog.*. This makes the
         * browse-only catalog a first-class SHARED data source driving the spares
         * newsvendor (spares page real-part pick + DCMOC fleet part-mix), layered
         * over the DATA.reliability + DATA.sparesPricing screening tables. ══ */
        sparesCatalog: {
            meta: { generated: 'tools/build-spares-catalog-aggregate.mjs', source: 'js/spares-parts-catalog.js', commodities: 12, partsAggregated: 339 },
            byCommodity: {
                chiller: { n: 16, mtbf: 12.25, mttr: 11.05, leadWk: 18, costUsd: 19600, alts: 1, crit: 8, eol: 6, life: { active: 11, nrnd: 4, ltb: 0, obsolete: 0 } },
                crac: { n: 24, mtbf: 10.03, mttr: 10.59, leadWk: 13, costUsd: 17250, alts: 1, crit: 5, eol: 5, life: { active: 20, nrnd: 4, ltb: 0, obsolete: 0 } },
                ups: { n: 15, mtbf: 9.33, mttr: 4.5, leadWk: 11, costUsd: 27400, alts: 1, crit: 8, eol: 5, life: { active: 13, nrnd: 1, ltb: 0, obsolete: 1 } },
                switchgear: { n: 45, mtbf: 17.4, mttr: 6.75, leadWk: 19, costUsd: 15200, alts: 1, crit: 8, eol: 6, life: { active: 32, nrnd: 10, ltb: 0, obsolete: 2 } },
                transformer: { n: 10, mtbf: 18.6, mttr: 33.9, leadWk: 30, costUsd: 39700, alts: 1, crit: 8, eol: 5, life: { active: 8, nrnd: 1, ltb: 0, obsolete: 0 } },
                generator: { n: 25, mtbf: 10.6, mttr: 7.06, leadWk: 15, costUsd: 11500, alts: 1, crit: 5, eol: 5, life: { active: 22, nrnd: 3, ltb: 0, obsolete: 0 } },
                pump: { n: 33, mtbf: 11, mttr: 7.02, leadWk: 13, costUsd: 7670, alts: 2, crit: 6, eol: 4, life: { active: 27, nrnd: 3, ltb: 0, obsolete: 1 } },
                valve: { n: 8, mtbf: 20.55, mttr: 4.52, leadWk: 6, costUsd: 995, alts: 2, crit: 5, eol: 3, life: { active: 7, nrnd: 0, ltb: 0, obsolete: 0 } },
                hx: { n: 35, mtbf: 10.1, mttr: 4.65, leadWk: 11, costUsd: 4790, alts: 1, crit: 6, eol: 5, life: { active: 34, nrnd: 1, ltb: 0, obsolete: 0 } },
                'cooling-tower': { n: 10, mtbf: 9.08, mttr: 5.33, leadWk: 9, costUsd: 3690, alts: 1, crit: 4, eol: 4, life: { active: 9, nrnd: 1, ltb: 0, obsolete: 0 } },
                bms: { n: 75, mtbf: 9.73, mttr: 2.86, leadWk: 7, costUsd: 1460, alts: 2, crit: 5, eol: 6, life: { active: 62, nrnd: 7, ltb: 0, obsolete: 3 } },
                fuel: { n: 9, mtbf: 8.83, mttr: 5.89, leadWk: 7, costUsd: 5030, alts: 3, crit: 4, eol: 4, life: { active: 9, nrnd: 0, ltb: 0, obsolete: 0 } },
                fire: { n: 34, mtbf: 12.75, mttr: 3.96, leadWk: 11, costUsd: 3110, alts: 1, crit: 5, eol: 5, life: { active: 28, nrnd: 5, ltb: 0, obsolete: 1 } }
            }
        },
        decision: {
        ...
        coolingArchProfiles: {
            direct_liquid:  { label: 'Direct Liquid (Warm-Water LTC)', rackType: 'ai_hpc_direct_liquid', liquidCapture: 85, supplyTemp: 32.5, returnTemp: 40.5, airCop: 4.6, fanPower: 2.8, economizerHours: 34, coefHeatTransfer: 94, coefPipeLoss: 1.0  },
            hybrid:         { label: 'Hybrid Liquid + Air',            rackType: 'enterprise_mixed',     liquidCapture: 55, supplyTemp: 24,   returnTemp: 32,   airCop: 4.7, fanPower: 4.1, economizerHours: 34, coefHeatTransfer: 89, coefPipeLoss: 1.02 },
            air_inrow_dahu: { label: 'Air-Cooling In-Row / DAHU',      rackType: 'legacy_air_plus_rdhx', liquidCapture: 12, supplyTemp: 18,   returnTemp: 24,   airCop: 3.8, fanPower: 5.0, economizerHours: 22, coefHeatTransfer: 76, coefPipeLoss: 1.06 }
        },
        ltcClimate: {
        ...
        ltcCalibration: {
            /* COP model */
            copBase:                  6.1,
            supplyBonusSlope:         0.11,
            supplyBonusRef:           28,
            supplyBonusClampMin:     -0.45,
            supplyBonusClampMax:      1.5,
            liquidCopMin:             3.0,
            liquidCopMax:             19,
            liquidCopEconFrac:        1.05,
            airCopMin:                1.7,
            airCopMax:                16,
            airCopEconFrac:           0.58,
            airCopCtrlFrac:           0.32,
            airResidualBase:          0.92,
            airResidualScale:         0.12,
            controlBonusDivisor:      180,
            controlBonusRef:          60,
            /* Hydraulic model */
            pumpDensityStress:        0.22,
            redundancyFactorN1:       1.15,
            redundancyFactor2N:       2,
            /* effectiveCapture */
            effectiveCaptureA:        0.74,
            effectiveCaptureB:        0.26,
            effectiveCaptureCtrlFrac: 500,
            /* density / future */
            densityStressRef:         70,
            densityStressMin:         0.5,
            densityStressMax:         2.8,
            designDensityHDSDiv:      250,
            designDensityYearSlope:   0.0045,
            futureBiasYearSlope:      0.9,
            futureFactorMin:          0.8,
            futureFactorMax:          1.65,
            /* deltaT */
            deltaTBase:               0.9,
            deltaTStressSlope:        0.08,
            deltaTMin:                3,
            deltaTMax:                20,
            /* fan */
            fanPowerLiquidRed:        0.55,
            fanPowerCtrlRed:          0.18,
            /* WUE model */
            wueEconFrac:              0.43,
            wueLiquidFrac:            0.32,
            wueFutureFrac:            0.18,
            wueMin:                   0.04,
            /* heat reuse */
            heatReuseFactorA:         0.42,
            heatReuseFactorB:         0.1,
            heatReuseYearGain:        0.012,
            heatReuseCredit:          0.62,
            avoidedCarbonFrac:        0.65,
            /* auxiliary */
            auxKwFraction:            0.03,
            /* control index */
            controlIndexWeightCq:     0.55,
            controlIndexWeightPg:     0.95,
            controlIndexWeightMon:    0.22,
            controlIndexConcurrent:   8,
            /* failure modes */
            sensorDegradedPenalty:    18,
            sensorDegradedRiskAdd:    13,
            redundancyDegradedRiskAdd: 16,
            gridStressElecMult:       1.18,
            gridStressCarbonMult:     1.12,
            gridStressRiskAdd:        6,
            heatwaveAirCopMult:       0.82,
            heatwaveAirCopMin:        1.4,
            heatwaveRiskAdd:          9,
            pumpDegradedPowerMult:    1.28,
            pumpDegradedCoolingMult:  1.14,
            pumpDegradedFlowMult:     0.9,
            pumpDegradedRiskAdd:      11,
            /* risk index */
            riskBase:                 18,
            riskNRedundancyAdd:       12,
            riskNoConcurrentAdd:      8,
            riskHighDensityThresh:    90,
            riskHighDensityAdd:       8,
            riskHighRackDensityThresh: 80,
            riskHighRackDensityAdd:   8,
            riskDryPipeAdd:           6,
            riskMonitoringSlope:      0.2,
            riskMonitoringRef:        80,
            riskUpsSlope:             3,
            riskUpsRef:               96,
            riskControlSlope:         0.18,
            riskControlRef:           70,
            riskYearSlope:            0.12,
            riskMin:                  5,
            riskMax:                  95,
            /* confidence */
            confidenceBase:           45,
            confidenceMonSlope:       0.32,
            confidenceConcurrentAdd:  6,
            confidenceCountryAdd:     8,
            confidenceDeltaTOk:       6,
            confidenceHydMarginOk:    3,
            confidencePipeOk:         2,
            /* score weights */
            totalScoreWeightAshrae:   0.24,
            totalScoreWeightAnsi:     0.16,
            totalScoreWeightIso:      0.16,
            totalScoreWeightNfpa:     0.20,
            totalScoreWeightUptime:   0.24,
            /* ASHRAE score thresholds */
            ashraeSupplyLow:          18,
            ashraeSupplyHigh:         40,
            ashraeSupplyPenalty:      30,
            ashraeReturnHigh:         50,
            ashraeReturnPenalty:      18,
            ashraeDeltaTLow:          6,
            ashraeDeltaTLowPenalty:   14,
            ashraeDeltaTHigh:         14,
            ashraeDeltaTHighPenalty:  8,
            ashraeCaptureLow:         65,
            ashraeCapturePenalty:     12,
            ashraeDensityHigh:        100,
            ashraeDensityPenalty:     7,
            ashraeControlLow:         60,
            ashraeControlPenalty:     9,
            /* ANSI score */
            ansiBase2N:               88,
            ansiBaseN1:               78,
            ansiBaseN:                62,
            ansiMonSlope:             0.11,
            ansiHighDensityThresh:    80,
            ansiHighDensityPenalty:   6,
            ansiHighHydMargin:        25,
            ansiHighHydPenalty:       4,
            ansiLegacyPenalty:        6,
            /* ISO score */
            isoBase:                  35,
            isoMonSlope:              0.42,
            ...
            isoPue4Bonus:             4,
            isoIntensity1:            250,
            isoIntensity1Bonus:       8,
            isoIntensity2:            350,
            isoIntensity2Bonus:       4,
            isoFutureFactor:          1.15,
            isoFutureBonus:           4,
            /* NFPA score */
            nfpaCleanAgent:           94,
            nfpaWaterMist:            88,
            nfpaDoubleInterlock:      81,
            nfpaDryPipe:              72,
            nfpaConcurrentAdd:        4,
            nfpaHighDensityThresh:    90,
            nfpaHighDensityPenalty:   4,
            /* Uptime score */
            uptimeBase2N:             92,
            uptimeBaseN1:             80,
            uptimeBaseN:              58,
        ...
        sources: {
            ...
            'sparesCatalog':        { source: 'COMPACT per-commodity p50 (median) aggregate of the curated 445-part DC spare-parts catalog (js/spares-parts-catalog.js), itself generated from the gitignored master DB data/spares-parts.sqlite by tools/build-spares-db.py (curated subset — real OEM part numbers, MTBF/MTTR, lead-time bands, list-price bands, qualified alternates, criticality, lifecycle state, EOL horizon). The engine aggregate is produced by tools/build-spares-catalog-aggregate.mjs: for each of 12 spares commodities it takes the median mtbf(yr)/mttr(hr)/ltTyp(wk)/cTyp(USD)/alts/crit/eol(months) + lifecycle mix over the parts in that commodity\'s catalog subsystem(s). Read via models.spares.catalog.* — feeds the newsvendor as a first-class SHARED data source (spares page real-part pick + DCMOC fleet part-mix), layered over the DATA.reliability + DATA.sparesPricing screening tables.', asOf: '2026-07', unit: 'p50 mtbf/mttr/lead/cost/alts/crit/eol per commodity + part count', method: 'median over curated catalog parts per commodity; SCREENING — a curated subset, not an OEM RCM study; regenerate when the catalog changes' },
            ...
            'ltcClimate':          { source: 'LTC System Modelling Lab internal climate-zone model (ltc-system-modelling-lab.js CLIMATE_FACTORS, v2026-Q2). Wet-bulb and base WUE bands calibrated to ASHRAE 169-2021 climate zones; climateCopBias from LTC model calibration vs published tropical/temperate air-cooled COP surveys (ASHRAE HoF 2021 ch.39).', asOf: '2026', unit: 'deg-C wet-bulb + WUE L/kWh + COP bias', method: 'model-calibration against ASHRAE HoF + LTC lab empirical' },
            'ltcCountryProfiles':  { source: 'LTC System Modelling Lab country profiles (ltc-system-modelling-lab.js COUNTRY_PROFILES, v2026-Q2). Electricity prices: IEA 2025 / PLN / EMA / utility filings; water tariffs: public utility schedules; carbon intensity: Ember grid-intensity 2025; COP/PUE targets from Uptime Global Survey 2026 cohort by climate zone.', asOf: '2026', unit: 'mixed — $/kWh, $/m3, kgCO2/kWh, ratio', method: 'SCREENING aggregate; replace with site-specific tariffs for a project' },
    ...
    return {
        ...
        totalOPEX: totalOPEX, recommend: recommend
            ...
            spares: {
                /** Curated 445-part catalog as a SHARED data source (v2.5.3).
                 *  Reads the COMPACT DATA.sparesCatalog aggregate (generated from
                 *  js/spares-parts-catalog.js by tools/build-spares-catalog-aggregate.mjs)
                 *  so the spares page AND DCMOC drive the newsvendor from real p50
                 *  catalog MTBF/MTTR/lead/cost/alts/crit, not just the screening tables.
                 *  Absent aggregate → null (callers fall back to screening). */
                catalog: {
                    /** All commodity keys present in the aggregate. */
                    commodities: function () {
                        var bc = DATA.sparesCatalog && DATA.sparesCatalog.byCommodity;
                        return bc ? Object.keys(bc) : [];
                    },
                    /** p50 aggregate for a commodity key → {n,mtbf,mttr,leadWk,costUsd,alts,crit,eol,life}
                     *  or null when no catalog parts map to that commodity. */
                    byCommodity: function (commodityKey) {
                        var bc = DATA.sparesCatalog && DATA.sparesCatalog.byCommodity;
                        return (bc && bc[commodityKey]) ? bc[commodityKey] : null;
                    },
                    /** Newsvendor-shaped inputs derived from the catalog aggregate for a
                     *  commodity: unitCost=costUsd, ltWeeks=leadWk, partLifeYrs≈designLife,
                     *  muAnnual = installedQty·(8760/(mtbf·8760)) = installedQty/mtbf.
                     *  Returns null when the commodity has no catalog aggregate. */
                    classAggregate: function (commodityKey, installedQty) {
                        var a = this.byCommodity(commodityKey);
                        if (!a) return null;
                        var qty = installedQty > 0 ? installedQty : 1;
                        var muAnnual = a.mtbf > 0 ? qty / a.mtbf : qty;
                        return {
                            commodity: commodityKey, n: a.n,
                            mtbfYrs: a.mtbf, mttrHrs: a.mttr,
                            unitCost: a.costUsd, ltWeeks: a.leadWk,
                            alts: a.alts, crit: a.crit, eolMonths: a.eol, life: a.life,
                            muAnnual: +muAnnual.toFixed(3),
                            provenance: 'catalog', source: 'DATA.sparesCatalog (p50 of curated 445-part catalog)'
                        };
                    }
                },
        ...
        /* ══ v2.6.0 — models.ltc: LTC system-modelling compute engine.
         * Exact same formulas as ltc-system-modelling-lab.js computeModel —
         * reads calibration constants from DATA.coolants / DATA.rackProfiles /
         * DATA.coolingArchProfiles / DATA.ltcClimate / DATA.ltcCalibration.
         * The LTC page calls this via window.RZEngine.models.ltc.compute(input). ══ */
        ltc: {
            compute: function (input) {
                function _clamp(v, lo, hi) { return v < lo ? lo : (v > hi ? hi : v); }

                var CAL = DATA.ltcCalibration;
                var climateData = DATA.ltcClimate[input.climate] || DATA.ltcClimate.tropical;
                var coolant = DATA.coolants[input.coolantKey] || DATA.coolants.water;
                var rackProfile = DATA.rackProfiles[input.rackType] || DATA.rackProfiles.ai_hpc_direct_liquid;
                ...
                var futureFactor = _clamp(1 + ((input.coefFutureTech + (yearDelta * CAL.futureBiasYearSlope)) / 100) * rackProfile.futureBias, CAL.futureFactorMin, CAL.futureFactorMax);
                var controlIndex = _clamp(
                    (input.controlQuality * CAL.controlIndexWeightCq) +
                    (input.predictiveGain * CAL.controlIndexWeightPg) +
                    (input.monitoring * CAL.controlIndexWeightMon) +
                    (input.concurrent ? CAL.controlIndexConcurrent : 0),
                    0, 100
                );

                if (failureMode === 'sensor_degraded') {
                    controlIndex = _clamp(controlIndex - CAL.sensorDegradedPenalty, 0, 100);
                    failureRiskAdd += CAL.sensorDegradedRiskAdd;
                    failureNote = 'Sensor degraded: reduced control observability';
                } else if (failureMode === 'redundancy_degraded') {
                    failureRiskAdd += CAL.redundancyDegradedRiskAdd;
                    failureNote = 'Redundancy degraded: resilience margin reduced';
                } else if (failureMode === 'grid_stress') {
                    elecPriceEff *= CAL.gridStressElecMult;
                    carbonIntensityEff *= CAL.gridStressCarbonMult;
                    failureRiskAdd += CAL.gridStressRiskAdd;
                    failureNote = 'Grid stress: tariff and carbon intensity elevated';
                }

                var designDensity = Math.max(5, input.rackDensityTarget * (1 + input.highDensityShare / CAL.designDensityHDSDiv) * (1 + yearDelta * CAL.designDensityYearSlope));
                var densityStress = _clamp(designDensity / CAL.densityStressRef, CAL.densityStressMin, CAL.densityStressMax);
                var effectiveCapture = _clamp(
                    input.liquidCapture * (CAL.effectiveCaptureA + rackProfile.liquidBias * CAL.effectiveCaptureB) * (input.coefHeatTransfer / 100) * (1 + controlIndex / CAL.effectiveCaptureCtrlFrac),
                    20, 99
                );
                ...
                var deltaT = _clamp(baseDeltaT * (CAL.deltaTBase + densityStress * CAL.deltaTStressSlope), CAL.deltaTMin, CAL.deltaTMax);

                var massFlowKgS = liquidKw / (coolant.cp * deltaT);
                var baseFlowM3s = massFlowKgS / coolant.rho;
                var redundancyFactor = input.redundancy === '2N' ? CAL.redundancyFactor2N : (input.redundancy === 'N1' ? CAL.redundancyFactorN1 : 1);
                ...
                var pumpHeadEff = input.pumpHead * pipeFactor * (1 + (densityStress - 1) * CAL.pumpDensityStress);
                ...
                var supplyBonus = _clamp((input.supplyTemp - CAL.supplyBonusRef) * CAL.supplyBonusSlope, CAL.supplyBonusClampMin, CAL.supplyBonusClampMax);
                var controlBonus = (controlIndex - CAL.controlBonusRef) / CAL.controlBonusDivisor;

                var liquidCop = _clamp(
                    (CAL.copBase + climateData.climateCopBias + supplyBonus + economizerFraction * CAL.liquidCopEconFrac + controlBonus) * futureFactor,
                    CAL.liquidCopMin, CAL.liquidCopMax
                );

                var airCopEff = _clamp(
                    input.airCop * (1 + economizerFraction * CAL.airCopEconFrac + controlBonus * CAL.airCopCtrlFrac) * (CAL.airResidualBase + (1 - rackProfile.airResidual) * CAL.airResidualScale),
                    CAL.airCopMin, CAL.airCopMax
                );

                if (failureMode === 'heatwave') {
                    airCopEff = _clamp(airCopEff * CAL.heatwaveAirCopMult, CAL.heatwaveAirCopMin, CAL.airCopMax);
                    failureRiskAdd += CAL.heatwaveRiskAdd;
                ...
                if (failureMode === 'pump_degraded') {
                    pumpPowerKw *= CAL.pumpDegradedPowerMult;
                    liquidCoolingKw *= CAL.pumpDegradedCoolingMult;
                    flowLpm *= CAL.pumpDegradedFlowMult;
                    failureRiskAdd += CAL.pumpDegradedRiskAdd;
                    failureNote = 'Pump degraded: lower flow and higher hydraulic overhead';
                }
                var airCoolingKw = airKw / airCopEff;
                var fanKw = itKw * (input.fanPower / 100) * (1 - effectiveCapture / 100 * CAL.fanPowerLiquidRed) * (1 - controlBonus * CAL.fanPowerCtrlRed);
                ...
                var auxKw = itKw * CAL.auxKwFraction;
                ...
                var wue = climateData.baseWue * (1 - economizerFraction * CAL.wueEconFrac) * (1 - effectiveCapture / 100 * CAL.wueLiquidFrac) * (1 - (futureFactor - 1) * CAL.wueFutureFrac);
                wue = Math.max(wue, CAL.wueMin);
                ...
                var heatReuseKwAvg = liquidKw * (input.heatReuse / 100) * (CAL.heatReuseFactorA + rackProfile.futureBias * CAL.heatReuseFactorB) * (1 + yearDelta * CAL.heatReuseYearGain);
                var heatReuseKwh = heatReuseKwAvg * 8760;
                var heatReuseCredit = heatReuseKwh * elecPriceEff * CAL.heatReuseCredit;
                var netOpex = Math.max(grossOpex - heatReuseCredit, 0);

                var annualCarbonTons = (annualKwh * carbonIntensityEff) / 1000;
                var avoidedCarbonTons = (heatReuseKwh * carbonIntensityEff * CAL.avoidedCarbonFrac) / 1000;
                ...
                /* ── Score functions (same logic as LTC IIFE helpers) ── */
                function _ashraeScore(supplyTemp, returnTemp, dT, effCapture, dDensity, ctrlIdx) {
                    var s = 100;
                    if (supplyTemp < CAL.ashraeSupplyLow || supplyTemp > CAL.ashraeSupplyHigh) s -= CAL.ashraeSupplyPenalty;
                    if (returnTemp > CAL.ashraeReturnHigh) s -= CAL.ashraeReturnPenalty;
                    if (dT < CAL.ashraeDeltaTLow) s -= CAL.ashraeDeltaTLowPenalty;
                    if (dT > CAL.ashraeDeltaTHigh) s -= CAL.ashraeDeltaTHighPenalty;
                    if (effCapture < CAL.ashraeCaptureLow) s -= CAL.ashraeCapturePenalty;
                    if (dDensity > CAL.ashraeDensityHigh) s -= CAL.ashraeDensityPenalty;
                    if (ctrlIdx < CAL.ashraeControlLow) s -= CAL.ashraeControlPenalty;
                    return _clamp(s, 0, 100);
                }
                function _ansiScore(redundancy, monitoring, rackDensityVal, hydraulicMargin, rackProfileKey) {
                    var base = redundancy === '2N' ? CAL.ansiBase2N : (redundancy === 'N1' ? CAL.ansiBaseN1 : CAL.ansiBaseN);
                    base += monitoring * CAL.ansiMonSlope;
                    if (rackDensityVal > CAL.ansiHighDensityThresh) base -= CAL.ansiHighDensityPenalty;
                    if (hydraulicMargin > CAL.ansiHighHydMargin) base -= CAL.ansiHighHydPenalty;
                    if (rackProfileKey === 'legacy_air_plus_rdhx') base -= CAL.ansiLegacyPenalty;
                    return _clamp(base, 0, 100);
                }
                function _isoScore(monitoring, pueVal, nCarbon, aGwh, fFactor) {
                    var s = CAL.isoBase + monitoring * CAL.isoMonSlope;
                    if (pueVal <= CAL.isoPue1) s += CAL.isoPue1Bonus;
                    else if (pueVal <= CAL.isoPue2) s += CAL.isoPue2Bonus;
                    else if (pueVal <= CAL.isoPue3) s += CAL.isoPue3Bonus;
                    else if (pueVal <= CAL.isoPue4) s += CAL.isoPue4Bonus;
                    var intensity = aGwh > 0 ? nCarbon / aGwh : 0;
                    if (intensity <= CAL.isoIntensity1) s += CAL.isoIntensity1Bonus;
                    else if (intensity <= CAL.isoIntensity2) s += CAL.isoIntensity2Bonus;
                    if (fFactor > CAL.isoFutureFactor) s += CAL.isoFutureBonus;
                    return _clamp(s, 0, 100);
                }
                function _nfpaScore(fireType, concurrent, dDensity) {
                    var map = { clean_agent: CAL.nfpaCleanAgent, water_mist: CAL.nfpaWaterMist, double_interlock: CAL.nfpaDoubleInterlock, dry_pipe: CAL.nfpaDryPipe };
                    var s = map[fireType] || 70;
                    if (concurrent) s += CAL.nfpaConcurrentAdd;
                    if (dDensity > CAL.nfpaHighDensityThresh) s -= CAL.nfpaHighDensityPenalty;
                    return _clamp(s, 0, 100);
                }
                function _uptimeScore(redundancy, monitoring, concurrent, upsEff, ctrlIdx) {
                    var s = redundancy === '2N' ? CAL.uptimeBase2N : (redundancy === 'N1' ? CAL.uptimeBaseN1 : CAL.uptimeBaseN);
                    s += monitoring * CAL.uptimeMonSlope;
                    s += (upsEff - CAL.uptimeUpsRef) * CAL.uptimeUpsSlope;
                    s += (ctrlIdx - CAL.uptimeCtrlRef) * CAL.uptimeCtrlSlope;
                    if (concurrent) s += CAL.uptimeConcurrentAdd;
                    return _clamp(s, 0, 100);
                }

                var scoreA = _ashraeScore(input.supplyTemp, input.returnTemp, deltaT, effectiveCapture, designDensity, controlIndex);
                var scoreB = _ansiScore(input.redundancy, input.monitoring, rackDensity, input.hydraulicMargin, input.rackType);
                var scoreC = _isoScore(input.monitoring, pue, netCarbonTons, annualGwh, futureFactor);
                var scoreD = _nfpaScore(input.fireType, input.concurrent, designDensity);
                var scoreE = _uptimeScore(input.redundancy, input.monitoring, input.concurrent, input.upsEff, controlIndex);
                var totalScore = (scoreA * CAL.totalScoreWeightAshrae) + (scoreB * CAL.totalScoreWeightAnsi) + (scoreC * CAL.totalScoreWeightIso) + (scoreD * CAL.totalScoreWeightNfpa) + (scoreE * CAL.totalScoreWeightUptime);

                var riskIndex = CAL.riskBase;
                riskIndex += rackProfile.riskBias;
                if (input.redundancy === 'N') riskIndex += CAL.riskNRedundancyAdd;
                if (!input.concurrent) riskIndex += CAL.riskNoConcurrentAdd;
                if (designDensity > CAL.riskHighDensityThresh) riskIndex += CAL.riskHighDensityAdd;
                if (rackDensity > CAL.riskHighRackDensityThresh) riskIndex += CAL.riskHighRackDensityAdd;
                if (input.fireType === 'dry_pipe') riskIndex += CAL.riskDryPipeAdd;
                riskIndex += Math.max(0, (CAL.riskMonitoringRef - input.monitoring) * CAL.riskMonitoringSlope);
                riskIndex += Math.max(0, (input.upsEff < CAL.riskUpsRef ? (CAL.riskUpsRef - input.upsEff) * CAL.riskUpsSlope : 0));
                riskIndex -= Math.max(0, (controlIndex - CAL.riskControlRef) * CAL.riskControlSlope);
                riskIndex -= yearDelta * CAL.riskYearSlope;
                riskIndex += failureRiskAdd;
                riskIndex = _clamp(riskIndex, CAL.riskMin, CAL.riskMax);

                var confidence = _clamp(
                    CAL.confidenceBase +
                    (input.monitoring * CAL.confidenceMonSlope) +
                    (input.concurrent ? CAL.confidenceConcurrentAdd : 0) +
                    (input.countryKey !== 'custom' ? CAL.confidenceCountryAdd : 0) +
                    ((deltaT >= 6 && deltaT <= 14) ? CAL.confidenceDeltaTOk : 0) +
                    ((input.hydraulicMargin <= 20) ? CAL.confidenceHydMarginOk : 0) +
                    ((input.coefPipeLoss >= 0.9 && input.coefPipeLoss <= 1.2) ? CAL.confidencePipeOk : 0),
                    0, 99
                );
