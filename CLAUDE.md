...
```
**AUTO-LINKING chain (owner mandate — engine changes must propagate to docs WITHOUT manual edits):** after ANY `rz-engine.js` change also run `node tools/build-engine-catalog.mjs` (regenerates `dcmoc/src/lib/engine-catalog.json` — models/functions/params/sources/consumers, grep-derived) then `node tools/test-value-bindings.mjs` (SHIP GATE: value-bindings coherence + catalog staleness — a changed engine with a stale catalog does NOT ship). The DCMOC Knowledge Base + FAQ render the catalog live, so regeneration = all doc surfaces current. See ENGINE_UNIFICATION.md §AUTO-LINKING CHAIN. **TRACE-PARITY GATE** (`node tools/_dcmoc_trace_parity_probe.mjs`): clicks every `[data-trace]` number on ~20 pages (incl. per-row inline-trace cells) and asserts the popover value matches the rendered KPI (fuzzy mantissa/scale) — catches mirror-node drift; run after any value-trace.ts or page-formula change (214/214 baseline as of v1.121.10 (Tax/Grid/Talent detail cells re-traced inline to their own country-scoped value; SiteIntel keeps the site-scoped nodes), whitelisted local-param nodes WARN only). **MODEL-CALIBRATION GATE** (`node tools/test-model-calibration.mjs`, SHIP GATE): validates engine constants against the LIVE corpus per `DATA.calibrationSpec` (single source shared with the DCMOC "Model Calibration" UI) — run after any change to `DATA.pueMatrix`/`capexPerMw`/`waterFootprint`, the spec, or a corpus regen; drift is a REPORTED finding, bands are never silently loosened (see `standarization/MODEL_CALIBRATION_STANDARD.md`). **ENUM-COVERAGE GATE** (`node tools/audit-dcmoc-enum-coverage.mjs`, SHIP GATE): every cost/value UI `<select>` option value must resolve to a real key in the engine DATA map it indexes (`CapexEngine` does `(cd.xMult ?? xMultipliers)[uiValue] || 1.0` — a missing key SILENTLY falls back to a neutral 1.0 / $1M default, mis-costing the choice). Extracts option domains from `CapexDashboard.tsx` + curated array-selects, checks against `DATA.capexDetail.*` / `DATA.markets` (cityMarket alias-aware via `resolveMarketKey`); 11/11 baseline, exit 1 on any gap. Run after adding/renaming any CAPEX/market select option OR any `DATA.capexDetail` map key. Caught nothing new after v1.115.62 fixed the last gaps — it exists to keep it that way (this bug class bit us in v1.115.61 market + v1.115.62 capex).
...
| `js/rz-diagram.js` + `js/rz-diagram-metrics.js` + `js/rz-diagram-layout.js` | RZDiagram — THE block-diagram engine. Measures text per character BEFORE drawing, sizes nodes from their own content, places labels by search, routes connectors orthogonally around non-endpoint boxes | **Every new block diagram goes through this.** Do NOT add hand-typed SVG coordinates — that is the defect it removes. Treat engine `warnings` (`node-overflow`, `label-unplaced`, `transit`) as failures of the LAYOUT, not of the engine. No literal hex: colours resolve to the page's own custom properties. Gates: `node tools/test-rz-diagram-engine.mjs` + `node tools/demo-rz-diagram.mjs` + `node tools/test-diagram-engine-adoption.mjs` (RATCHET — hand-authored geometry per cockpit file may fall freely, a rise FAILS; re-record with `--baseline` after a migration). See standarization/DIAGRAM_ENGINE_STANDARD.md. |
...
- **`standarization/DIAGRAM_ENGINE_STANDARD.md` — READ BEFORE drawing or editing any block diagram.** Why the engine exists (the drawing code could not ask how wide a label is), what it guarantees (per-character width budget, placement by search, orthogonal routing, labels painted last), and the two `diagram-design` §4 anti-patterns this brand deliberately overrides with reasons written down. Skin is brand; geometry is not negotiable.
