...
const ROOT='/home/baguspermana7/rz-work'; const PORT=8111;
...
// click the Internal Staff Cost KPI button
const clicked=await page.evaluate(()=>{ const btn=[...document.querySelectorAll('button')].find(x=>/^\s*\$/.test(x.textContent||'') && x.title && /trace how Internal/i.test(x.title)); if(btn){btn.click(); return btn.title;} return null; });
await new Promise(r=>setTimeout(r,500));
const modal=await page.evaluate(()=>{ const t=document.body.innerText; return { hasTrace:/Trace — how this is calculated/i.test(t), hasHow:/How it is calculated/i.test(t), title:(t.match(/Internal Staff Cost[^\n]*/)||[''])[0] }; });
console.log('clicked:',clicked); console.log('modal:',JSON.stringify(modal));
