...
const GROUPS_PER_ROW = SNAPSHOT.geometry.rack_groups_per_row;
...
function testBasisIsReadNotTyped() {
  ...
  assert.throws(
    () => Electrical.build({
      ...
      geometry: { rack_groups_per_hall: GROUP_COUNT, racks_per_group: RACKS_PER_GROUP + 1, rack_groups_per_row: GROUPS_PER_ROW, rack_rows: 1, racks_per_row: 1 },
    }),
...
function testTopologyContract() {
  ...
  assert.equal(byId.get('TX-A').unitCount, SNAPSHOT.equipment.transformers_per_hall_per_feed);
  assert.equal(byId.get('TX-A').unitMva, SNAPSHOT.equipment.transformer_unit_mva);
  assert.equal(byId.get('UPS-B').frameCount, SNAPSHOT.equipment.ups_frames_per_hall_per_feed);
