...
def main() -> int:
    ...
    db_text = ('/* GENERATED by tools/build-explain-db.py — DO NOT EDIT BY HAND.\n'
    ...
    if '--check' in sys.argv:
        stale = [name for name, path, text in
                 ((OUT.name, OUT, db_text),
                  ('search-terms.json', ROOT / 'search-terms.json', terms_text))
                 if not path.exists() or path.read_text(encoding='utf-8') != text]
        if stale:
            print('FAIL ' + ', '.join(stale) + ' stale against glossary.html + the curated '
                  'batches; regenerate with --apply')
            return 1
        print(f'Current: {OUT.name} ({len(merged)} entries), search-terms.json ({len(terms)})')
        return 0

    OUT.write_text(db_text, encoding='utf-8')
    (ROOT / 'search-terms.json').write_text(terms_text, encoding='utf-8')
