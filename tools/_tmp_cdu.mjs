...
const ROOT='/home/baguspermana7/rz-work'; const PORT=8104;
...
const evap=await page.evaluate(()=>{ const t=document.body.innerText; return { hasWater:/Water\s*&\s*Glycol Balance/i.test(t), annual:(t.match(/Annual Water[\s\S]{0,30}?([\d,]+)\s*m³/)||[])[1], wue:(t.match(/WUE\s*([\d.]+)\s*L\/kWh/)||[])[1] }; });
// switch to dry rejection and re-read
const opts=await page.evaluate(()=>{ for(const s of document.querySelectorAll('select')){ if([...s.options].map(o=>o.value).includes('dry')){ const set=Object.getOwnPropertyDescriptor(window.HTMLSelectElement.prototype,'value').set; set.call(s,'dry'); s.dispatchEvent(new Event('change',{bubbles:true})); return true; } } return false; });
...
console.log('evaporative:',JSON.stringify(evap),'| drySwitch:',opts);
