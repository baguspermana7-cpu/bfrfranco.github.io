...
def write_js_catalog(parts_rows):
    ...
    mode_recs = [dict(zip(["id","name","interTyp","interMin","interMax","intraTyp","costIndex","co2Index","capUnit","use","notes"],
                         (m[0],m[1],(float(m[2]) if m[2] is not None else None),m[3],m[4],m[5],m[6],m[7],m[8],m[9],m[10]))) for m in TRANSPORT_MODES]
    lane_recs = [dict(zip(["id","origin","dest","mode","oceanD","airD","roadRailD","customsD","lastMileD","congestion","geopolitical","volatility","tariff","reroute","notes"], l)) for l in TRADE_LANES]
    country_recs = [dict(zip(["country","region","polStab","customs","port","lpi","transformerSharePct","geoRisk","tariffNote","notes"], c)) for c in COUNTRY_RISK]
    payload = {
        ...
        "transportModes": mode_recs,
        "tradeLanes": lane_recs,
        "countryRisk": country_recs,
    }
