   ...
   real identities, not token mentions: each line is a relation the engine must satisfy. */
{
  ...
  ok(S.geometry.rack_groups_per_row === S.geometry.racks_per_row / S.geometry.racks_per_group, 'geometry.rack_groups_per_row = racks per row / racks per group');
  ok(S.geometry.integer_grouping === true && S.geometry.rack_rows * S.geometry.racks_per_row === M.facility.racksPerHall &&
     S.geometry.rack_groups_per_hall * S.geometry.racks_per_group === M.facility.racksPerHall,
     'geometry.integer_grouping: rows x racks/row = groups x racks/group = racks/hall');
  ...
  ok(S.distribution.rack_groups_per_hall === Ge.rack_groups_per_hall, 'distribution.rack_groups_per_hall agrees with the floor grid');
  ...
  ok(S.distribution.busway_within_trunk === (Di.group_current_a <= M.electrical.buswayTrunkA) && Di.busway_within_trunk === true,
     'distribution.busway_within_trunk: the group current fits the declared trunk rating', Di.busway_loading_pct);
  ...
  ok(S.distribution.hall_group_kw_check === true && Di.group_kw * Di.rack_groups_per_hall === S.power.rack_it_hall_kwe,
     'distribution.hall_group_kw_check: groups x group kW = hall rack IT');

  ok(E.transformers_per_hall_per_feed === Math.ceil(E.facility_kva / (M.facility.halls * 2) / (M.equipment.transformer.unitMva * 1000)),
     'equipment.transformers_per_hall_per_feed = ceil(kVA per hall per feed / unit)');
  ok(E.ups_frames_per_hall_per_feed === Math.ceil(S.power.total_it_kwe / M.facility.halls / (M.equipment.ups.unitKw * M.equipment.ups.designLoadingMax)),
     'equipment.ups_frames_per_hall_per_feed = ceil(hall IT / (frame x design loading))');
  ok(E.gensets_facility_shared === true, 'equipment.gensets_facility_shared: the pool is sized on the facility load, with no per-hall split');
